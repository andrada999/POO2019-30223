...
public class AquaticFactory extends SpeciesFactory {
	public Animal getAnimal(String type) throws Exception {
		...
		double maintenanceCost = 4.5;
		double dangerPerc = 0.5;
		...
		if (Constants.Animals.Aquatics.Shark.equals(type)) {
			return new Shark(0, aquaticNames[name], maintenanceCost,dangerPerc, swimDepth, waterType[water]);
		} else if (Constants.Animals.Aquatics.Dolphin.equals(type)) {
			return new Dolphin(0, aquaticNames[name],maintenanceCost,dangerPerc, swimDepth, waterType[water]);
		} else if (Constants.Animals.Aquatics.Octopus.contentEquals(type)) {
			return new Octopus(8, aquaticNames[name], maintenanceCost,dangerPerc, swimDepth, waterType[water]);
		} else
