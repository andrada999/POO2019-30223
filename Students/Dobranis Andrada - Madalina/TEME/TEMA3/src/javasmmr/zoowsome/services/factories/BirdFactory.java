...
public class BirdFactory extends SpeciesFactory {
	public Animal getAnimal(String type) throws Exception {
		...
		if (Constants.Animals.Birds.Pigeon.equals(type)) {
			return new Pigeon(2, birdNames[name], migrationState, altitude);
		} else if (Constants.Animals.Birds.Parrot.equals(type)) {
			return new Parrot(2, birdNames[name], migrationState, altitude);
		} else if (Constants.Animals.Birds.Colibri.contentEquals(type)) {
			return new Colibri(2, birdNames[name], migrationState, altitude);
		} else
